...
titleOG: ""
titleTwitter: ""
titleCourse: "Music generation"
courseCategoryName: "Audio analysis"
desc: "Poof, another creative profession gone"
descSEO: ""
descOG: ""
descTwitter: ""
date: "09.02.2024"
...
otherTags: [""]
keywordsSEO: [""]
banner: "../../../images/posts/research/banners/music_generation.jpg"
imageOG: ""
imageAltOG: ""
imageTwitter: ""
imageAltTwitter: ""
canonicalURL: "https://avrtt.github.io/research/music_generation"
slug: "/research/music_generation"
---
...
5. Deep learning for audio-based music generation  
...
7. Applications and tools  
    ...
    Project Music GenAI Control (Adobe), AIVA, Amper Music, etc.  
    Creative and educational applications  
    Integration into music production workflows  
8. Research groups, labs, and authors  
    Active research labs (Google Magenta, Metacreation Lab, Audiolabs Erlangen, etc.)  
...
(!) TO-DO:
9. Suno and Udio: in-depth look on the most popular music generation models in 2025  
    Review of architectures, design, details, methods, technologies, etc.  
    Differences between Suno and Udio  
10. More technical part (code, math, etc.)
...
In the 1990s, we saw a confluence of ideas in artificial neural networks and algorithmic composition. One of the noteworthy developments was <Highlight>HarmoNet</Highlight> (1992). HarmoNet was an early neural system focusing on generating chordal accompaniments to existing melodies. It used a neural approach to harmonize Bach chorales in a style reminiscent of traditional four-part writing. The performance was promising for its time, though hampered by the relatively limited computational resources of that era.
...
1. **Computational load**: Generating each sample is a sequential process, which is extremely slow for extended durations.
2. **Long-term structure**: While local continuity can be captured effectively, maintaining large-scale musical coherence is difficult without additional conditioning signals or memory mechanisms.
...
1. **Idea generation**: For brainstorming quick melodic or harmonic sketches.
2. **Layer or track creation**: Generating backing tracks or transitional sections that are later refined by a human musician.
3. **Sound design**: Using neural network transformations to shape or morph audio, akin to advanced effects or creative sampling.
...
When training a model to predict the next token in a sequence, a common objective is **cross-entropy loss**. If <Latex text="\(p_\theta(y_t \mid y_{t-1}, \dots, y_1)\)"/> is the model's predicted probability of token <Latex text="\(y_t\)"/> at time <Latex text="\(t\)"/> given the previous tokens and parameters <Latex text="\(\theta\)"/>, and <Latex text="\(y_t^*\)"/> is the true token, then the loss for one time step can be written as:
...
In this context:
- <Latex text="\(y_t^*\)"/> is the ground truth token at time <Latex text="\(t\)"/>.
- <Latex text="\(p_\theta\)"/> is modeled by the neural network (e.g., LSTM) outputs, typically fed through a softmax layer.
...
<Image alt="Symbolic representation of a piano roll" path="" caption="A typical piano roll representation showing time on one axis and pitch on the other, used for training symbolic models" zoom="false" />

<Image alt="Diagram of a simple LSTM network unrolled over time" path="" caption="An unrolled view of an LSTM, showing hidden states being passed forward as the model processes a sequence of tokens" zoom="false" />

<Image alt="Transformer architecture diagram with multi-head self-attention" path="" caption="A generic visualization of transformer layers highlighting the self-attention mechanism" zoom="false" />

<Image alt="High-level diagram of a GAN for music generation" path="" caption="Generator and discriminator in a GAN setup for symbolic or audio music generation" zoom="false" />

<Image alt="Bar-based generation strategy" path="" caption="Illustration of how bar-level chunks can be generated and stitched together in certain transformer approaches" zoom="false" />
