...
- Synthetic data generation: Describes creating instruction–response pairs from smaller "seed" tasks using advanced models (e.g., GPT-4) and system prompts.  
...
1. **Pre-training**: The model is trained on a massive collection of unstructured text, relying on language modeling objectives — for instance, predicting the next token given the context (<Latex text="\(p(x_{t} \mid x_{1}, \ldots, x_{t-1})\)"/>). This is often done on multi-terabyte text corpora. The goal is to develop broad linguistic and world knowledge. This phase can be extremely computationally expensive, frequently employing hundreds or thousands of GPUs over extended periods of time.  
...
4. **Policy gradient update**: Update the policy parameters <Latex text="\(\theta\)"/> with PPO, which modifies the log probabilities <Latex text="\(\log p_\theta\)"/> of the generated tokens, aiming to maximize the expected reward while avoiding drastic changes to the policy. The latter is enforced via a clipped objective that prevents large policy updates:
   ...
   \]"/>
   where <Latex text="\(r_t(\theta)\)"/> is the ratio between the new and old policy probabilities for an action (token), and <Latex text="\(\hat{A_t}\)"/> is an advantage estimate. <Latex text="\(\epsilon\)"/> is a hyperparameter (clip range), typically around 0.1–0.2.  
