...
indexCourse: 144
indexFavorites:
title: "Intro to AI theory, pt. 1"
titleDetailed: ""
titleSEO: ""
titleOG: ""
titleTwitter: ""
titleCourse: "Intro to AI theory, pt. 1"
...
imageTwitter: ""
imageAltTwitter: ""
canonicalURL: "https://avrtt.github.io/research/intro_to_ai_theory"
slug: "/research/intro_to_ai_theory"
---
...
Artificial Intelligence: A Modern Approach, Global Edition, 4ed: chapters 1 + 28 + 29
https://alexanderdyakonov.wordpress.com/2018/08/28/%d0%b8%d0%bd%d1%82%d0%b5%d1%80%d0%bf%d1%80%d0%b5%d1%82%d0%b0%d1%86%d0%b8%d0%b8-%d1%87%d1%91%d1%80%d0%bd%d1%8b%d1%85-%d1%8f%d1%89%d0%b8%d0%ba%d0%be%d0%b2/
https://www.cs.cornell.edu/courses/cs4780/2024sp/slides/ethics.pdf
Интерпретируемые модели
https://neerc.ifmo.ru/wiki/index.php?title=%D0%98%D0%BD%D1%82%D0%B5%D1%80%D0%BF%D1%80%D0%B5%D1%82%D0%B8%D1%80%D1%83%D0%B5%D0%BC%D1%8B%D0%B5_%D0%BC%D0%BE%D0%B4%D0%B5%D0%BB%D0%B8
...
Motivation and Philosophical Underpinnings
- Why we need theoretical grounding in AI
- The engineer–theorist gap
- Bridging empirical progress with rigorous foundations
- Why engineering outpaces theoretical development; translating rapid empirical breakthroughs into formal proofs, importance of reproducible and interpretable research    
- Philosophical foundations: defining intelligence, philosophical vs. computational perspectives
...
- Search algorithms (breadth-first, depth-first, backtracking, etc.) and constraint satisfaction problems
- Constraint propagation
- Backtracking search
- Alpha-Beta Pruning and Game-Tree Search (Minimax search, theoretical bounds)
- Limitations of purely symbolic approaches
- Formal Automata Theory for reasoning and decision-making
- Temporal Logic in AI Planning (e.g., LTL, CTL)
- Applications in scheduling, verification, and planning
- Automata theory and its early impact on AI, applications of formal automata theory
...
- Approximation algorithms (designing algorithms that provide near-optimal solutions for intractable AI problems)
- Parameterized complexity (analyzing AI problems based on multiple complexity parameters, such as size of input or structure of data)
...
- Kolmogorov complexity as a measure of "intelligence" or model simplicity
- Minimum description length (MDL) principle in AI (a formalization of Occam's Razor in model selection)
- Universal distribution and universal inference approaches
- Compression-Based Learning: Analyzing AI through its ability to compress data effectively.
...
- Probabilistic logic and graphical models (brief introduction)
...
Meta-learning, few-show and zero-shot (meta learning will be covered in details in the future articles, here we need introduction)
...
### alpha-beta pruning and game-tree search
...
- **LTL**: Typically used to express properties along a single linear path through time. One might write <Latex text="\( \square (safe) \)"/> to mean "always safe" or <Latex text="\( \lozenge (reach\_goal) \)"/> to mean "eventually the goal is reached."
...
- **Fine-grained complexity**: Investigating the exact time complexities (like <Latex text="\( O(n^{2.1}) \)"/> vs. <Latex text="\( O(n^2) \)"/>) for certain tasks can significantly impact how we scale algorithms.
...
Formally, the Kolmogorov complexity <Latex text="\( K(x) \)"/> of a string <Latex text="\( x \)"/> is defined as:

<Latex text="\[
K(x) = \min_{p : U(p)=x} |p|
\]"/>

where <Latex text="\( U \)"/> is a universal Turing machine, <Latex text="\( p \)"/> is a program such that <Latex text="\( U(p)=x \)"/>, and <Latex text="\( |p| \)"/> is the length of that program in bits. However, <Latex text="\( K(x) \)"/> itself is uncomputable in the general case, implying we can't directly measure it. But conceptually, it provides a guiding principle: AI that can find shorter descriptions for data might exhibit better generalization or deeper "understanding" of the patterns.  
...
The universal distribution, introduced by Solomonoff (1964), is a probability distribution over strings that is dominated by short descriptions on a universal Turing machine. If <Latex text="\( m(x) \)"/> denotes the universal a priori probability of string <Latex text="\( x \)"/>, then:

<Latex text="\[
m(x) = \sum_{p: U(p)=x} 2^{-|p|}
\]"/>

where the sum is over all programs <Latex text="\( p \)"/> that output <Latex text="\( x \)"/>. This distribution, while not computable in practice, theoretically encapsulates all possible patterns in <Latex text="\( x \)"/>. Solomonoff induction and related universal inference frameworks assume that the environment's data can be described by some computable distribution, and thus short programs are more likely. This forms a foundation for a theoretically optimal predictor, albeit uncomputable in practice.

From an AI viewpoint, universal inference underscores the notion that an ideal learner would search among all possible descriptions of data, weighting them by <Latex text="\( 2^{-|p|} \)"/>. Modern machine learning methods approximate this vision with hierarchical, deep architectures that attempt to discover features from data. They can sometimes be viewed as a partial search for shorter encodings or more robust representations.  
...
<Image alt="conceptual representation of ai theory foundations" path="" caption="Illustration of various foundational theories in AI: from symbolic to sub-symbolic, from logic to probability." zoom="false" />
