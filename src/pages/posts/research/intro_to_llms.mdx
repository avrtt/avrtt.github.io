...
**LLM engineer roadmap перенесён в посты про AI engineering**

# LLMs for scientists

Здесь и в следующих нескольких постах мы будем говорить про LLM с точки зрения исследователя. Более практические моменты для LLM-инженеров будут разобраны в будущих постах, поскольку для них требует освоить множество других тем.
...
Recurrent neural networks (RNNs) marked a watershed moment. Introduced in earlier decades by Elman and Jordan, RNNs allow hidden states to be updated over sequences, effectively capturing an unbounded history — at least in theory. An RNN's hidden state at time step <Latex text="\(t\)"/> is typically defined as:
