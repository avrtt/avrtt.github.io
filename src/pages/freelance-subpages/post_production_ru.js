...
const projectsFiltered = projects.filter(item => item.categoryKey === 'post_production')
const categoryDescDetailedRu = categories.find(item => item.categoryKey === 'post_production').categoryDescDetailedRu;

projectsFiltered.sort(function (a, b) {
	return b.id - a.id;
  });

const TITLE = 'Проекты по пост-продакшену - avrtt.blog'

const PostProductionRu = () => {
...
export default PostProductionRu;
