...
const projectsFiltered = projects.filter(item => item.categoryKey === 'misc')
const categoryDescDetailedRu = categories.find(item => item.categoryKey === 'misc').categoryDescDetailedRu;

projectsFiltered.sort(function (a, b) {
	return b.id - a.id;
  });

const TITLE = 'Прочие проекты - avrtt.blog'
  ...
  return (
    <motion.div
		...
		<div class="categorySwitchWrapper">
			...
			<Link title="Проекты по веб-разработке" class="categoryButton noselect" to="/freelance/ru/projects/web_development"><div class="categoryButton noselect"><img src={WebDevelopmentIcon} alt="button_icon" width="50%" height="100%" /></div></Link>
		...
		<div class="freelanceBody">
			...
			/>
			<br/>
			<M text="В данном списке представлены мои проекты, которые не были определены ни в одну из категорий (исключая вклад в open-source и прочие проприетарные или небольшие проекты):"/>
